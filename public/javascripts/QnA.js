            ...
            for_info_div.innerHTML = '';
                ...
                answer_see.addEventListener('click', () => {    //답글보기 눌렀을 때.
                    if (control == 0) {
                        ...
                        fetch('/QnA/answer/' + content.id, { method: 'GET' })
                            .then((response) => {
                                if (response.status == '200') {
                                    return response.json();
                                }
                            ...
                            .then((resJson) => {
                                const answer_infos = resJson;
                                    answer_see_content.innerHTML = answer_infos.answer;
                ...
                answer.id = "answer";   //form 형식으로. 관리자일 때 답글달기 버튼(사실 텍스트) 생기도록.

                if (user_u_nickName) {
                    if (user_u_nickName.value == content.q_nick) {
                        ...
                        answer_form.action = '/QnA/' + content.id;
                        ...
                        answer.addEventListener('click', () => {    //답글 달기 눌렀을 때 밑에 form 뜨도록.
